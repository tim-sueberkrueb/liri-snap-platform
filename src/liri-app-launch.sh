...
if [ -d $SNAP/liri-app-platform ]; then
  ...
  if [ ! -d $RUNTIME/usr ] ; then
    echo "You need to connect this snap to the liri app platform snap."
    echo ""
    echo "You can do this with those commands:"
    echo "snap install liri-app-platform"
    echo "snap connect $SNAP_NAME:platform liri-app-platform:platform"
    exit 1
  fi
fi
